...
function kapee_setup_loop( $args = array() ) {
	...
	$default_args = array(
		'name'          						=> 'posts-loop',
		//Blog
		'post-fancy-date'          				=> kapee_get_option( 'post-fancy-date', 1 ),
		...
		'specific-post-meta'          			=> kapee_get_option( 'specific-post-meta', array( 'post-author', 'post-category', 'post-comments', 'post-share' ) ),
		'post-meta-icon'          				=> kapee_get_option( 'post-meta-icon', 1 ),
		'post-meta-separator'          			=> kapee_get_option( 'post-meta-separator', 'meta-separator-colon' ),
		//Blog Archive
		'blog-post-style'          				=> kapee_get_option( 'blog-post-style', 'blog-bottom-overlap' ),
		'blog-grid-layout'          			=> kapee_get_option( 'blog-grid-layout', 'simple-grid' ),
		'blog-grid-post-style'          		=> kapee_get_option( 'blog-grid-post-style', 'blog-grid-bottom-overlap' ),
...
function kapee_body_classes( $classes ) {
	...
	if( kapee_get_option('mobile-bottom-navbar', 1 ) ) { 
		if( ( function_exists('is_product') && is_product() && !kapee_get_option( 'mobile-product-page-button', 1 ) ) 
			|| ( function_exists('is_cart') && is_cart() && !kapee_get_option( 'mobile-cart-page-button', 1 ) ) 
			|| ( function_exists('is_checkout') && is_checkout() && !kapee_get_option( 'mobile-checkout-page-button', 1 ) ) ){
			$classes[] = '';
		}else{
...
if( ! function_exists( 'kapee_mobile_menu' ) ) {
	...
	function kapee_mobile_menu() {
		...
		<div id="mobile-menu-wrapper" class="mobile-menu-wrapper">
			...
			if( KAPEE_WOOCOMMERCE_ACTIVE ){
				...
				<div class="mobile-menu-header">
					<?php if( ! is_user_logged_in() ):?>
						<a class="customer-signinup" href="<?php echo ( kapee_get_option('login-register-popup', 1) ) ? 'javaScript:void(0)' : esc_url($dashboard_url);?>"><?php echo esc_html($mobile_signup_text);?></a>
					<?php else:?>
			...
			<?php if( has_nav_menu( $primary_menu_location ) || has_nav_menu( $categories_menu_location ) ){ ?>
				<div class="mobile-nav-tabs">
					<ul>
						...
						<?php if ( kapee_get_option('show-categories-menu', 1 ) && has_nav_menu( 'categories-menu' ) ) { ?>
			...
			if ( kapee_get_option('show-categories-menu', 1 ) && has_nav_menu( $categories_menu_location ) ) {
				wp_nav_menu( array( 
	...
	add_action( 'kapee_body_bottom', 'kapee_mobile_menu', 100 );
}
/**
 * Header
 */
...
function kapee_is_empty_column( $col ){
	return empty( array_filter( $col ) );
}
function kapee_is_empty_elements( $el ){
...
if ( ! function_exists( 'kapee_close_popup' ) ) :

	/**
	 * Close sidebar popup.
	 */
	function kapee_close_popup() {?>
...
if ( ! function_exists( 'kapee_template_single_post_related' ) ) {
	...
	function kapee_template_single_post_related( $args = array() ) {
		...
		kapee_set_loop_prop( 'specific-post-meta', array( 'post-author', 'post-comments', 'post-share' ) );
...
if ( ! function_exists( 'kapee_mobile_bottom_navbar' ) ) {	
	...
	function kapee_mobile_bottom_navbar(){
		...
		if( ! kapee_get_option( 'mobile-bottom-navbar', 1 ) ) return; 
		...
		foreach ( $mobile_elemets['enabled'] as $element => $value ) {
			...
			switch ( $element ) {
				...
				case 'cart':
					...
					$element_args['icon'] 	= 'icon-handbag';
				...
				case 'compare':
					...
					$element_args['link'] 	= '#';
					$element_args['icon'] 	= 'icon-shuffle';
				...
				case 'sidebar':
					...
					if( kapee_is_catalog() ){												
						$element_args['link'] 	= '#';
						$element_args['icon'] 	= 'icon-equalizer';
						...
						$element_args['link'] 	= '#';
						$element_args['icon'] 	= 'icon-notebook';
				...
				case 'search':
					$element_args['link'] 	= '#';
					$element_args['icon'] 	= 'icon-magnifier';
			...
			$args['elements'][$element] = apply_filters( 'kapee_mobile_navbar_element'.$element, $element_args );
			
		}
		
		kapee_get_template( 'template-parts/mobile/mobile-navbar.php',$args);			
	}
}