			...
			<div id="review_form">
				<?php
					...
					if ( $account_page_url ) {
						...
						$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( esc_html__( 'You must be %slogged in%S to post a review.', 'kapee' ), '<a href="' . esc_url( $account_page_url ) . '">', '</a>' ) . '</p>';
					}

					if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
						$rating_style = kapee_get_option('products-rating-style','fancy-rating'); //Kapee
						$comment_form['comment_field'] = '<div class="comment-form-rating '.esc_attr($rating_style).'"><label for="rating">' . esc_html__( 'Your rating', 'kapee' ) . '</label><select name="rating" id="rating" required>
